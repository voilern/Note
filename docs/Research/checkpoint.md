...
本文采用一种启发式算法对计算图染色，相较于构造冲突图 (conflicting graph) $O(N^2)$ 的复杂度，只需要 $O(N)$ 时间即可完成。具体而言，该算法按拓扑序遍历计算图，并使用计数器记录每个节点的 liveness。当没有其它待处理操作依赖于其输入时可以进行原地操作，当该算法中的回收后的 tag 被其它节点复用时可以进行内存共享。下图展示了算法的细节。
