...
此时 $a = 3,\; b = 4,\; f(N) = \Theta(N^2)$，即每次分为 3 个子问题，子问题的规模为 $N/4$，合并开销为 $\Theta(N^2)$。由于此例下分治的策略是相对均匀的，所以我们可以认为得到的是一个完美三叉树。下一页的例子中收缩因子不一致，导致两侧树高不相同，从而会带来新的问题。

显然树高为 $\log_4 N$，根记为 0，每个分治节点的 combine 开销已经标注在图的节点位置，横向箭头标记的是对该层所有节点的开销的求和。特别的，对于最底层，即叶子层，它表示的是 conquer 部分的开销。于是我们可以根据下式的形式对其进行求和，得到图片中下方的式子。
...
需要注意的是，这三种情况并没有覆盖 $f(N)$ 的全部可能性。1 和 2，2 和 3 之间都有一定间隙，如果 $f(N)$ 落在两个间隙之中，或者 3 所要求的正则条件不成立，就不能使用定理一求解递归式。
